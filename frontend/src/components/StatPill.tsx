...
export default function StatPill({ label, value, delta, up, accent, neutral }: StatPillProps) {
  ...
  return (
    ...
    >
      ...
      {delta && (
        <span style={{ fontSize: '0.75rem', fontWeight: 600, color: neutral ? 'var(--text-muted)' : up ? 'var(--accent)' : 'var(--red)' }}>
          {neutral ? '' : up ? '↑ ' : '↓ '}{delta}
        </span>
      )}
    </div>
