...
const milestones = [
  ...
  { year: '2025', text: 'Achieved first full year of sustained profitability — €17B in revenue, 751M users, and positive operating income for all four quarters. The growth-at-all-costs era is over.' },
]
...
const businessCards = [
  {
    icon: '📈',
    title: 'Growth Engine',
    body1: "Spotify's freemium model attracts users through a free ad-supported tier, lowering barriers to entry and converting listeners into premium subscribers.",
    body2: 'This strategy scaled Spotify to over 751 million users worldwide.',
  },
  {
    icon: '💰',
    title: 'Revenue Model',
    body1: 'Premium subscriptions drive ~89% of revenue. With 290 million subscribers and an ARPU of €4.63, subscriptions are the core monetization engine.',
    body2: 'High royalty and licensing costs, however, significantly compress profit margins.',
  },
  {
    icon: '🏆',
    title: 'Competitive Advantage',
    body1: "Spotify's two-sided platform creates strong network effects — more listeners attract more creators, and more content attracts more users.",
    body2: 'AI-powered features like Discover Weekly deepen engagement and differentiate Spotify in a crowded market.',
  },
  {
    icon: '⚖️',
    title: 'Strategic Tension',
    body1: 'Spotify faces a core tension: massive scale but thin margins, squeezed by high licensing costs and competitive pressure.',
    body2: "Balancing growth with profitability remains Spotify's defining strategic challenge.",
  },
]
  ...
  return (
    <div id="revenue" style={{ marginTop: 48 }} ref={ref}>
        ...
        {activeTab === 0 && (
          <div>
            <p style={{ margin: '0 0 24px', fontSize: '0.85rem', color: 'var(--text-dim)', lineHeight: 1.7 }}>
              Premium subscriptions drive 89% of Spotify's revenue — advertising accounts for just 11%, despite serving 461 million free-tier users. That gap isn't a structural given; it's an opportunity. Closing it is one of the clearest paths to margin expansion without touching royalty rates.
            </p>
            {bars.map(b => (
      ...
      <p className="insight" style={{ marginTop: 24 }}>
        <strong>Spotify cracked profitability in 2025 — but the structural forces that compressed margins for a decade haven't gone away. That tension is what the rest of this analysis is about.</strong>
      </p>
    </div>
  )
}
      ...
      <div ref={cards.ref} className={`reveal${cards.vis ? ' visible' : ''}`}>
        ...
        <GlassCard style={{ margin: '48px 0 32px', padding: '40px 48px', borderLeft: '4px solid var(--accent)', background: 'linear-gradient(135deg, rgba(29,185,84,0.07) 0%, rgba(29,185,84,0.02) 100%)', position: 'relative', overflow: 'hidden' }}>
          ...
          <p style={{ fontSize: '1rem', color: 'var(--text)', lineHeight: 1.75, maxWidth: 780, margin: '0 0 14px', fontWeight: 500 }}>
            Spotify didn't just build a streaming service — it restructured how the music industry works.
            By combining behavioral data at scale with a freemium funnel no competitor has replicated,
            Spotify transformed music from a product you own into a service you live inside.
          </p>
          <p style={{ fontSize: '0.95rem', color: 'var(--text-dim)', lineHeight: 1.75, maxWidth: 780, margin: 0 }}>
            That shift is why the company matters strategically — and why the constraints it now faces are worth examining closely.
          </p>
        </GlassCard>
        ...
        <div style={{ marginTop: 48 }}>
          ...
          <GlassCard style={{ padding: '32px 36px' }}>
            {/* Header row */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 72px 72px 72px 72px', gap: '0 8px', marginBottom: 20, paddingBottom: 14, borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
              <span style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Capability</span>
              {['V', 'R', 'I', 'O'].map((h, i) => (
                <div key={h} style={{ textAlign: 'center' as const }}>
                  <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase' }}>{h}</div>
                  <div style={{ fontSize: '0.6rem', color: 'var(--text-muted)', marginTop: 2 }}>{['Valuable','Rare','Inimitable','Organized'][i]}</div>
                </div>
              ))}
            </div>
            {[
              { name: 'Behavioral Data at Scale',    desc: '750M+ users generating first-party listening data competitors cannot replicate.',              vrio: ['full','full','full','full']    },
              { name: 'Freemium Conversion Engine',  desc: 'Proven funnel converting free listeners to paid — widely attempted, rarely replicated.',        vrio: ['full','full','partial','full'] },
              { name: 'Two-Sided Network Effects',   desc: 'Listeners and creators reinforce each other; every new user adds value to the platform.',       vrio: ['full','partial','full','full'] },
              { name: 'AI Personalization Stack',    desc: 'Built on Echo Nest (2014) and 18+ years of behavioral data — a durable, compounding edge.',    vrio: ['full','full','full','full']    },
            ].map(c => (
              <div key={c.name} style={{ display: 'grid', gridTemplateColumns: '1fr 72px 72px 72px 72px', gap: '0 8px', alignItems: 'center', marginBottom: 18 }}>
                <div>
                  <div style={{ fontSize: '0.88rem', fontWeight: 600, color: 'var(--text)', marginBottom: 3 }}>{c.name}</div>
                  <div style={{ fontSize: '0.76rem', color: 'var(--text-dim)', lineHeight: 1.5 }}>{c.desc}</div>
                </div>
                {c.vrio.map((s, idx) => (
                  <div key={idx} style={{ textAlign: 'center' as const, fontSize: '1.1rem', color: s === 'full' ? '#1db954' : s === 'partial' ? '#fbbf24' : '#3a3a44' }}>
                    {s === 'full' ? '●' : s === 'partial' ? '◑' : '○'}
                  </div>
                ))}
              </div>
            ))}
            <p style={{ margin: '8px 0 0', fontSize: '0.68rem', color: 'var(--text-muted)', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 12 }}>
              Assessed against VRIO criteria: Valuable · Rare · Inimitable · Organized
            </p>
          </GlassCard>
        </div>
